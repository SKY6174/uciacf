...
const NAV_ITEMS: Array<{ label: string; icon: Icon; active?: boolean; badge?: string }> = [
  { label: "통합 IR 대시보드", icon: LayoutDashboard, active: true },
...
function NavItem({ item, onClick }: { item: (typeof NAV_ITEMS)[number]; onClick: () => void }) {
  const ItemIcon = item.icon;
  return (
    <button className={`nav-item${item.active ? " active" : ""}`} onClick={onClick}>
      ...
      <aside className={`sidebar${sidebarOpen ? " open" : ""}`}>
        ...
        <nav aria-label="주요 메뉴">
          ...
          {NAV_ITEMS.slice(0, 4).map((item) => (
            <NavItem key={item.label} item={item} onClick={() => setSidebarOpen(false)} />
          ))}
